...
const ProgressScreen = () => {
  ...
  // Düello geçmişini yükle (aktivDuellolar tablosundan)
  useEffect(() => {
    const loadDuelHistory = async () => {
      if (user?.id) {
        try {
          const history = await DuelService.getAktifDuelloHistory(user.id, 50);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {/* Maç Geçmişi */}
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>⚔️ Maç Geçmişi</Text>
        {duelHistory.length === 0 ? (
          <View style={styles.emptyDuels}>
            <Ionicons name="game-controller-outline" size={48} color={theme.colors.textSecondary} />
            <Text style={styles.emptyDuelsText}>Henüz düello oynamadın</Text>
            <Text style={styles.emptyDuelsSubtext}>Arkadaşlarınla düello yaparak burada görüntüle!</Text>
          </View>
        ) : (
          duelHistory.slice(0, 10).map((duel, index) => {
            // AktifDuello formatına göre hesapla
            ...
            return (
              ...
              >
                ...
                <View style={styles.duelHistoryContent}>
                  <View style={styles.duelHistoryHeader}>
                    <Text style={styles.duelOpponentName}>{opponentName || 'Rakip'}</Text>
                    <Text style={[
                      styles.duelResultText,
                      { color: isDraw ? '#f59e0b' : isWinner ? '#22c55e' : '#ef4444' }
                    ]}>
                      {isDraw ? 'Berabere' : isWinner ? 'Kazandın' : 'Kaybettin'}
                    </Text>
                  </View>
                  <View style={styles.duelHistoryFooter}>
                    <Text style={styles.duelScore}>
                      {myScore || 0} - {opponentScore || 0}
                    </Text>
                    <Text style={styles.duelDate}>
                      {duel.createdAt ? new Date(duel.createdAt).toLocaleDateString('tr-TR') : ''}
                    </Text>
                  </View>
                </View>
            ...
            {selectedDuel && (() => {
              ...
              return (
                <ScrollView style={styles.modalScroll}>
                  ...
                  <View style={[
                    styles.resultBanner,
                    { backgroundColor: isDraw ? '#f59e0b20' : isWinner ? '#22c55e20' : '#ef444420' }
                  ]}>
                    <Ionicons 
                      name={isDraw ? 'remove-circle' : isWinner ? 'trophy' : 'sad'} 
                      size={48} 
                      color={isDraw ? '#f59e0b' : isWinner ? '#22c55e' : '#ef4444'} 
                    />
                    <Text style={[
                  ...
                  <View style={styles.scoreContainer}>
                    <View style={styles.playerScore}>
                      ...
                      <Text style={styles.scoreBig}>{myScore || 0}</Text>
                    </View>
                    <Text style={styles.vsText}>VS</Text>
                    <View style={styles.playerScore}>
                      <Text style={styles.playerName}>{opponentName || 'Rakip'}</Text>
                      <Text style={styles.scoreBig}>{opponentScore || 0}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.detailsContainer}>
                    <View style={styles.detailRow}>
                      ...
                      <Text style={styles.detailValue}>{selectedDuel.category || 'Karışık'}</Text>
                    </View>
                    <View style={styles.detailRow}>
                      <Text style={styles.detailLabel}>Soru Sayısı</Text>
                      <Text style={styles.detailValue}>{selectedDuel.questions?.length || 0}</Text>
                    </View>
                    <View style={styles.detailRow}>
                      <Text style={styles.detailLabel}>Tarih</Text>
                      <Text style={styles.detailValue}>
                        {selectedDuel.createdAt 
                          ? new Date(selectedDuel.createdAt).toLocaleDateString('tr-TR', {
                              day: 'numeric',
                              month: 'long',
                              year: 'numeric',
                              hour: '2-digit',
